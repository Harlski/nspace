...
and **Size** `20%..100%` in `10%` steps (default `100%`).
...
Attention Marker tool is active; walk mode never picks the marker. While that tool is active,
the client shows an **Attention Marker Selectability Cue** (fixed-accent floor tint under
each marked tile, stronger when that marker is selected) so pickability is visible without
changing server state or the marker tint.
...
surface for Buildings dock; tint the floor using the marker's hue for the selectability cue —
confuses “recolor the V” with “this tile is pickable,” so the cue uses a fixed build accent.
