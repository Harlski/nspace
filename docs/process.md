...
| `PAYMENT_INTENT_API_SECRET` | server + payment-intent-service | **Sidecar:** required; `Authorization: Bearer …` on `/v1/*`. **Game server:** same value enables `/admin/system` probe of `/v1/meta/features` **and** JWT **`/api/payment/*`** proxy to the sidecar (never expose to browsers). |
...
| `PAYMENT_INTENT_SERVICE_URL` | server | Base URL of the payment-intent sidecar for **`/admin/system`** probes **and** **`/api/payment/*`** proxy (e.g. `http://127.0.0.1:3090` or `http://payment-intent:3090` in Compose) |

**Payment intents (player, JWT)** in [`server/src/paymentIntentRoutes.ts`](../server/src/paymentIntentRoutes.ts): `POST /api/payment/intents`, `GET /api/payment/intents/:intentId`, `POST /api/payment/intents/:intentId/verify` — **`503`** `payment_intent_unavailable` if `PAYMENT_INTENT_SERVICE_URL` or `PAYMENT_INTENT_API_SECRET` is unset on the game server.
