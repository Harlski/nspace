...
| `DAILY_STATS_TELEGRAM_ENABLED` | server | Optional toggle for the **end-of-day stats Telegram report** (UTC). Defaults **on** when Telegram is configured; `0`/`false` disables, `1`/`true` forces on. Sent shortly after 00:00 UTC for the day that just ended: unique sign-ins, new users, **Nimiq Pay vs other**, NIM paid out, and **active in-game time** (capped-gap estimate, excludes long AFK). Manual trigger: `POST /api/admin/daily-stats/send?day=YYYY-MM-DD&send=1` (analytics admin wallet). **`/admin/system`** also has a **Send last 24h to Telegram** button (rolling 24h window; `POST /api/admin/system/daily-stats/send`, `preview=1` to preview only; system admin wallet) |
