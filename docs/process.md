...
**Nimiq Pay tutorial** (env `TUTORIAL_ENABLED`, `TUTORIAL_BUILDER_ALLOWLIST`, `TUTORIAL_FAUCET_AMOUNT_LUNA`, `TUTORIAL_DOOR_AMOUNT_LUNA`, `TUTORIAL_DOOR_RECIPIENT`; client `VITE_TUTORIAL_ESCAPE_MS`, `VITE_TUTORIAL_ESCAPE_COUNTDOWN_MS`): first-contact lesson for Pay mini-app sessions. Runtime on/off at **`/admin/settings`** (persisted in `admin-runtime-settings.json`; env `TUTORIAL_ENABLED=0` hard-disables regardless). Rooms `tutorial` (live) and `tutorial-staging` (authoring) are hidden from the room catalog. Player JWT routes: `GET /api/tutorial/door-quote`, `POST /api/tutorial/door-sent`, `POST /api/tutorial/unstick`, `POST /api/tutorial/abandon`. WebSocket `welcome.tutorial` carries per-wallet lesson state. Pay band uses **Unlock Pad** (`TerrainProps.unlockPad`; per-wallet grants in `unlock-pad-grants.json`; optimistic proof in tutorial). Non-tutorial pads default to **Payment Intent** (`POST /api/unlock-pad/intent`, `POST /api/unlock-pad/sync`; feature kind `nspace.unlock_pad`). Admins place/edit via WS `placeUnlockPad` / `setUnlockPadConfig` (build dock Buildings). Deleting a pad clears its grants. Template records persist in `tutorial-templates.json` ([server/src/tutorialTemplate/store.ts](../server/src/tutorialTemplate/store.ts)); admin routes mirror Play Space templates (`/api/admin/tutorial-templates/*`). See [PRD-nimiq-pay-tutorial.md](PRD-nimiq-pay-tutorial.md), [PRD-unlock-pad.md](PRD-unlock-pad.md), [adr/0007-unlock-pad.md](adr/0007-unlock-pad.md).
