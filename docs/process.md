...
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | server | Optional — ping on **new** feedback tickets, **username set** (`Name Update: wallet -> name`), **Connect Notices** (fresh wallet auth or guest invite claim only; enriched player context; max one per wallet/guest per 15 minutes), and the **end-of-day stats report** (not the authoritative store) |
