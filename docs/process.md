...
**Nimiq Pay tutorial** (env `TUTORIAL_ENABLED`, `TUTORIAL_BUILDER_ALLOWLIST`, `TUTORIAL_FAUCET_AMOUNT_LUNA`, `TUTORIAL_DOOR_AMOUNT_LUNA`, `TUTORIAL_DOOR_RECIPIENT`; client `VITE_TUTORIAL_ESCAPE_MS`, `VITE_TUTORIAL_ESCAPE_COUNTDOWN_MS`): first-contact lesson for Pay mini-app sessions. Rooms `tutorial` (live) and `tutorial-staging` (authoring) are hidden from the room catalog. Player JWT routes: `GET /api/tutorial/door-quote`, `POST /api/tutorial/door-sent`, `POST /api/tutorial/unstick`, `POST /api/tutorial/abandon`. WebSocket `welcome.tutorial` carries per-wallet lesson state. Template records persist in `tutorial-templates.json` ([server/src/tutorialTemplate/store.ts](../server/src/tutorialTemplate/store.ts)); admin routes mirror Play Space templates (`/api/admin/tutorial-templates/*`). See [PRD-nimiq-pay-tutorial.md](PRD-nimiq-pay-tutorial.md).
