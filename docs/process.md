...
- **Private whispers** (`whisper` in → recipient, `whisper` out → sender echo) reuse the same profanity censor, 256-char cap, and shared `RATE_CHAT_MS` gate as public chat, but are **server-routed** to a single recipient (never broadcast, no 3D bubble). The target is resolved from `toName` (globally-unique **custom** username only, via `findWalletByCustomUsername`), `toAddress` (right-click / active chip), or `reply` (per-connection `lastWhisperPartner`, updated on both send and receive so `/r` replies in either direction). **Online-only** and **wallet players only**: guests and offline/unknown/self targets are rejected with WS errors `whisper_guest`, `whisper_offline`, `whisper_no_target`, `whisper_no_reply_target`, `whisper_self`. Successful whispers log a **`whisper`** event ([`ANALYTICS_EVENT_KINDS.whisper`](../server/src/eventLog.ts)) with censored `text`, optional `textOriginal`, `fromName`, `toAddress`, `toName`, and `at` for admin history; a received whisper is reportable via the existing **Report Message** flow.
- **Moderation store** ([server/src/moderationStore.ts](../server/src/moderationStore.ts), default `server/data/moderation.json`, override **`MODERATION_STORE_FILE`**): username-set bans, channel mutes, and **mining restrictions** (`miningBanned` with optional admin `note`). Game-admin JWT: `POST /api/admin/moderation` (`username_ban`, `channel_mute`, `mining_ban`, `clear_username`, `set_username`); snapshot `GET /api/admin/moderation` (alias `GET /api/admin/bans`). In-game profile Actions dropdown and server page **`/admin/moderation`** ([server/src/adminModerationPage.ts](../server/src/adminModerationPage.ts)). Mining restrictions block `beginBlockClaim` / `completeBlockClaim` ([server/src/blockClaimAccess.ts](../server/src/blockClaimAccess.ts)) and **hold block-claim pay-intents in the outbox / Payout Service queue** ([server/src/payoutMiningGate.ts](../server/src/payoutMiningGate.ts), [payout-service/src/miningBanGate.ts](../payout-service/src/miningBanGate.ts)); guest wallets never receive block-claim NIM.
...
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | server | Optional — ping on **new** feedback tickets, **username set** (`Name Update: wallet -> name`), **Connect Notices** (every game WebSocket connect; enriched player context including custom display name; stream observers omitted; max one per wallet/guest per minute), and the **end-of-day stats report** (not the authoritative store) |
...
| `NIM_PAYOUT_DEV_FAKE_BALANCE` | server | Local/dev only — when `1`/`true`, if the Payout Service balance cannot be read, serve a fake hot-wallet balance (default 100 NIM via `NIM_PAYOUT_DEV_FAKE_BALANCE_NIM`) for HUD + claim fund-gating. Live balance wins when the sidecar responds. Enabled by default via `npm run dev` / `ensure-payout-dev-env`. Do not enable in production. |
| `NIM_PAYOUT_PRIVATE_KEY` | payout only | Hot-wallet signer key — **never** on the game server |
| `NIM_PAYOUT_AUTO_BULK_AFTER_MS` | payout only | When the **oldest** pending job for a recipient has waited at least this long, auto **Payout in full** for that wallet (same path as admin Quick payout). Default **28800000** (8 hours); **`0`** disables |
| `NIM_PAYOUT_AUTO_BULK_CHECK_MS` | payout only | Scan interval for stale pending jobs (default **300000**, 5 min) |
| `NIM_TX_CONFIRM_TIMEOUT_MS` | payout only | Max wait for on-chain inclusion when sending (default **120000**). The game server uses this (+ buffer) as the HTTP timeout for manual bulk / flush calls |
...
- `PUT /api/admin/rooms/:id` — patch `displayName`, `isPublic`, `backgroundHueDeg`/`backgroundNeutral`, `joinSpawn` (dynamic rooms only), `builderAddresses` (dynamic **and** built-in rooms), and `ownerAddress` (player rooms only: transfer ownership to another wallet; rejected for built-in and official rooms); routes to `updateDynamicRoomMetadata` / `patchBuiltinRoomSettings`, persists, then `broadcastRoomCatalogRefresh()`.
...
**Nimiq Pay tutorial** (env `TUTORIAL_ENABLED`, `TUTORIAL_BUILDER_ALLOWLIST`, `TUTORIAL_FAUCET_AMOUNT_LUNA`, `TUTORIAL_DOOR_AMOUNT_LUNA`, `TUTORIAL_DOOR_RECIPIENT` — when unset, falls back to `ADVERTISE_FUND_RECIPIENT_ADDRESS` / its built-in default; client `VITE_TUTORIAL_ESCAPE_MS`, `VITE_TUTORIAL_ESCAPE_COUNTDOWN_MS`): first-contact lesson for Pay mini-app sessions. **Off by default** (env unset/`0` and admin `tutorialEnabled` default false). To go live: set `TUTORIAL_ENABLED=1` and enable the toggle at **`/admin/settings`**. When on, new and incomplete Pay wallets are forced into Tutorial Room on WebSocket connect (Hub/`chamber` after completion or when off). Admins may still teleporter-target and enter Tutorial Room while the learner flow is off. Rooms `tutorial` (live, shared) and `tutorial-staging` (admin authoring only) are hidden from the room catalog. Layout authored in **Tutorial Staging** is published via `/api/admin/tutorial-templates/:id/sync`; syncing the **default** template also reloads the live Tutorial Room and broadcasts geometry to connected players (`POST /api/admin/tutorial/reload-runtime` does the same). Edits only in staging are invisible to learners until sync. Player JWT routes: `GET /api/tutorial/door-quote`, `POST /api/tutorial/door-sent`, `POST /api/tutorial/unstick`, `POST /api/tutorial/abandon`, `POST /api/tutorial/reset-progress` (JWT — clears that wallet’s session/completion and Tutorial Room Unlock Pad grants so the lesson can restart at Mine; client shows **Start over** on the Tutorial Step Coach and **Reset tutorial** in the Player Menu while in `tutorial`). WebSocket `welcome.tutorial` carries per-wallet lesson state. Pay band uses **Unlock Pad** (`TerrainProps.unlockPad`; per-wallet grants in `unlock-pad-grants.json`; optimistic proof in tutorial). Non-tutorial pads default to **Payment Intent** (`POST /api/unlock-pad/intent`, `POST /api/unlock-pad/sync`; feature kind `nspace.unlock_pad`). Admins place/edit via WS `placeUnlockPad` / `setUnlockPadConfig` (build dock Buildings). Deleting a pad clears its grants. Template records persist in `tutorial-templates.json` ([server/src/tutorialTemplate/store.ts](../server/src/tutorialTemplate/store.ts)); admin routes mirror Play Space templates (`/api/admin/tutorial-templates/*`). To force the live Tutorial Room onto the code bootstrap shell: offline/dev `npm run tutorial:reseed-bootstrap:local` (rewrites `server/data/tutorial-templates.json` **and deletes** persisted `rooms/tutorial.json` / `tutorial-staging.json`, then **restart** the game server — template alone is skipped when room geometry already exists); remote `ADMIN_JWT=<admin jwt> npm run tutorial:reseed-bootstrap -- --url https://nimiq.space` (`POST /api/admin/tutorial/reseed-bootstrap`, then reloads runtime). Tutorial / Tutorial Staging stay **hidden from the Rooms teleporter catalog**; join via Finish tutorial / Start over / lesson entry, or `?room=tutorial` when allowed. See [PRD-nimiq-pay-tutorial.md](PRD-nimiq-pay-tutorial.md), [PRD-unlock-pad.md](PRD-unlock-pad.md), [adr/0007-unlock-pad.md](adr/0007-unlock-pad.md).
