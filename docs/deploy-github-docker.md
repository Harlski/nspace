...
- **`/admin/settings` 404 on Vercel:** the settings page is served by the game server; add the **`/admin/settings`** rewrite to the same `vercel.json` files (proxy to `https://api.nimiq.space/admin/settings`) and redeploy. **`GET/PUT /api/admin/settings`** is already covered by **`/api/:path*`** once you are on the page.
- **`/admin/feedback` 404 on Vercel:** same pattern — add **`/admin/feedback`** → API host in repo-root [`vercel.json`](../vercel.json) and [`client/vercel.json`](../client/vercel.json). Player **`/api/feedback/*`** and admin **`/api/admin/feedback/*`** are already proxied via **`/api/:path*`**.
- **`/advertise/uploads/...` 404 on Vercel:** campaign images are stored as same-origin paths on the SPA host; add **`/advertise/uploads/:path*`** (and **`/advertise`**, **`/advertise/how-it-works`**, **`/admin/campaign`**) rewrites to `https://api.nimiq.space/...` in both `vercel.json` files, then redeploy. Upload API **`POST /api/advertise/campaigns/upload-image`** is already covered by **`/api/:path*`**; only the static file GET needs the extra rewrite.
- **CORS + 502 on `/api/player-profile/username-prompt` (or other `/api/*`) from the SPA:** browsers often report **CORS** when the upstream API returns **502** without `Access-Control-Allow-Origin`. Prefer **same-origin** API calls on `nimiq.space` (leave **`VITE_API_BASE_URL` unset** on Vercel; client [`apiBase.ts`](../client/src/net/apiBase.ts) forces this on the SPA host). Confirm the game server is up: `curl -sS -w "\nhttp_code=%{http_code}\n" https://api.nimiq.space/api/health` and `curl -sS -H "Authorization: Bearer x" https://api.nimiq.space/api/player-profile/username-prompt` (expect **401**, not **502**). Deploy the latest server image if the route is missing (register **`username-prompt`** before **`/api/player-profile/:address`**).
