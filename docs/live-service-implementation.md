...
- **SPA (e.g. Vercel):** Build the `client` workspace. Repo-root [`vercel.json`](../vercel.json) runs `npm run build -w client` with `outputDirectory: client/dist` and rewrites `/api/*`, `/nim-chart-api/*`, and selected server HTML routes (`/admin`, `/admin/system`, `/admin/header`, `/admin/settings`, `/admin/feedback`, `/analytics`, `/payouts`, `/pixels.png`, …) to the production API host (`https://api.nimiq.space/...` in this repo’s config). **`/patchnotes`** (and `/patchnotes/`) rewrite to **`/index.html`** so the static host does not 404 on that client-only route. Player and admin JSON APIs (e.g. **`/api/feedback/*`**, **`/api/player-profile/username-prompt`**) are covered by **`/api/:path*`** — only server-served HTML admin pages need explicit rewrites besides that catch-all. If Vercel **Root Directory** is `client`, use [`client/vercel.json`](../client/vercel.json) instead and avoid conflicting dashboard overrides.
