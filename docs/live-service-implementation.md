...
- **Payment intents (optional):** Compose service `payment-intent` (profile **`payment`**) builds [`payment-intent-service`](../payment-intent-service/) as its own image, listens on `127.0.0.1:3090`, and persists SQLite under host `./data/payment-intent/`. The game server proxies **player** calls with JWT to **`/api/payment/*`** when `PAYMENT_INTENT_SERVICE_URL` and `PAYMENT_INTENT_API_SECRET` are set (server-to-sidecar still uses the shared secret only on the server). Details: [docker-deployment.md](docker-deployment.md).
...
GitHub Actions workflow [`.github/workflows/deploy-docker.yml`](../.github/workflows/deploy-docker.yml): on push to `main`, SSH to the server, `git fetch` + `git reset --hard origin/main`, then `docker compose build` and `docker compose up -d`. Setup: [deploy-github-docker.md](deploy-github-docker.md).
