...
- **Live Events:** `POST /api/live-events` on the game API (`https://api.nimiq.space` in production). Auth is a **NimiqLIVE Wallet** JWT (`LIVE_EVENT_ADDRESSES`), same Hub/Pay sign-in as players. Accepted Live Event Ids and operator type→interaction mappings persist in `server/data/live-events.sqlite` on the `data/` volume. System admins edit mappings at `/admin/live-events`.
