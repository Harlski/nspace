...
- **HTTP**: Express on `PORT` (default `3001`) — e.g. `/api/health`, **`GET /api/ambient-cast`** (public UTC-day Face Token snapshot for the Main Menu), `/api/auth/nonce`, `/api/auth/verify`, **`POST /api/live-events`** (NimiqLIVE Live Event ingest; wallet JWT allowlist), **`/admin/live-events`** (operator type → interaction mapping), `/api/header-marquee` (public in-game banner: streak leaderboard + `newsMessages[]` rotation, dwell/fallback seconds from JSON settings; **scroll layout and loop detection are client-side** in [client/src/ui/headerMarquee.ts](../client/src/ui/headerMarquee.ts)), `/api/admin/header-marquee` (admin), `/admin/header` (HTML) ([server/src/index.ts](../server/src/index.ts)).
...
Typical server → client messages include: `welcome` (includes **`chatBacklog`**: recent non-bubble room chat for reconnect / room changes, and **`moveOrders`**: in-flight Path Playback walks), `state` / `stateDelta` (presence cues are one-player `stateDelta`; grid Path Playback walkers omit pose on those deltas), `moveOrder` (analytic start + `walkId` + `serverNowMs`) / `moveAbort` / **`poseHeartbeat`** (~1 Hz analytic pose + `walkId` + `walking` for grid walkers), `obstacles` / `obstaclesDelta` (tiles may include **`signboardId`** when a signpost sits on that tile), `extraFloor` / `extraFloorDelta` (tiles may include **`colorRgb`**), **`baseFloorColorDelta`** (core-grid floor tints), `playerJoined` / `playerLeft`, `chat`, `signboards`, `billboards`, `roomBackgroundHue`, `roomJoinSpawn` (dynamic room entry tile), **`gateWalkBlocked`** (opener-only after `openGate` when the hinge opens but walking across is blocked), **`mosquitoTag`** (Tag Call / Tag Round snapshot; also on `welcome.mosquitoTag`), **`liveWorldEffect`** (Live Boost from a NimiqLIVE Live Event; also on `welcome.liveWorldEffect`), **`joinRoomFailed`** (`not_found` | `forbidden` | **`tag_round`** when a Participant is under Tag Room Lock), and room-specific payloads — see [client/src/main.ts](../client/src/main.ts) dispatch and [docs/features-checklist.md](features-checklist.md).
