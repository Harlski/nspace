...
Typical server → client messages include: `welcome` (includes **`chatBacklog`**: recent non-bubble room chat for reconnect / room changes), `state` / `stateDelta`, `obstacles` / `obstaclesDelta` (tiles may include **`signboardId`** when a signpost sits on that cell), `extraFloor` / `extraFloorDelta`, `playerJoined` / `playerLeft`, `chat`, `signboards`, `billboards`, `roomBackgroundHue`, `roomJoinSpawn` (dynamic room entry tile), **`gateWalkBlocked`** (opener-only after `openGate` when the hinge opens but walking across is blocked), and room-specific payloads — see [client/src/main.ts](../client/src/main.ts) dispatch and [docs/features-checklist.md](features-checklist.md).
