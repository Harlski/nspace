...
| [patchnote/README.md](patchnote/README.md) | **Patch notes** — `patchnote/versions/<version>/` ([UNRELEASED/reasons.md](patchnote/versions/UNRELEASED/reasons.md) + [public/](patchnote/versions/UNRELEASED/public/) tiers **Brief → Developers**; optional **`04-hotfix.md`**) |
...
**Patch notes:** each **patch-notes version** has its own folder `patchnote/versions/<version>/`. Append technical detail to **`reasons.md`** and audience-specific public copy to **`public/*.md`** for the same version (**Brief** through **Developers**; add **`public/04-hotfix.md`** only for hotfix narratives—see [docs/THE-LARGER-SYSTEM.md](docs/THE-LARGER-SYSTEM.md)). While developing, use **`patchnote/versions/UNRELEASED/`**. **During a feature** (incremental capture into `UNRELEASED/public/*.md`), follow **[`.cursor/skills/completed-feature-document/SKILL.md`](.cursor/skills/completed-feature-document/SKILL.md)** when the user says **CFD** / **Use completed-feature-document**. **Hotfix-style** copy: **[`.cursor/skills/hotfix-release-notes/SKILL.md`](.cursor/skills/hotfix-release-notes/SKILL.md)** (**HRN** / **Use hotfix release notes**). When the user is **preparing to merge to `main`** (e.g. says **“Prepare for merge”**, **PPA**, or **Use PPA**): **first** follow the audience checklist in **[`.cursor/skills/prepare-patchnotes-audience/SKILL.md`](.cursor/skills/prepare-patchnotes-audience/SKILL.md)** (finalize `UNRELEASED/public/*.md`, remove draft placeholders), **then** run **`npm run prepare-merge`** from the repo root so **`UNRELEASED`** is frozen under the next semver and **`package.json`** stays aligned (see [docs/THE-LARGER-SYSTEM.md](docs/THE-LARGER-SYSTEM.md) and [patchnote/README.md](patchnote/README.md)). Stub: [patchnote/reasons_UNRELEASED.md](patchnote/reasons_UNRELEASED.md).
