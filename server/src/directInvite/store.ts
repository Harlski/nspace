...
import { evaluateRedeem, reduceInvite } from "./reducer.js";
import type {
...
function dispatch(slug: string, event: InviteEvent): DirectInviteRecord | null {
  ...
  if (next) {
    ...
    if (next.guestId) slugByGuestId.set(next.guestId, slug);
  } else {
...
export type CreateInviteResult =
  ...
  | { ok: false; code: "host_busy" };

export function createInvite(input: CreateInviteInput): CreateInviteResult {
  const existing = getInviteByHost(input.hostWallet);
  if (existing && existing.phase !== "cancelled" && existing.phase !== "expired" && existing.phase !== "started") {
    return { ok: false, code: "host_busy" };
  }
...
export function claimInvite(
  ...
  if (invite!.guestId === guestId) {
    dispatch(slug, { type: "reclaim", guestId });
...
export function setInviteNickname(
  ...
  const invite = getInviteBySlug(slug);
  if (!invite || invite.guestId !== guestId) return null;
...
export function markGuestJoinedLobby(slug: string, guestId: string): DirectInviteRecord | null {
  const invite = getInviteBySlug(slug);
  if (!invite || invite.guestId !== guestId) return null;
...
export function startInviteMatch(slug: string): DirectInviteRecord | null {
  const invite = getInviteBySlug(slug);
  if (!invite || invite.phase !== "lobby") return null;
  return dispatch(slug, { type: "hostStart" });
}

export function markInviteStarted(slug: string): DirectInviteRecord | null {
  const invite = getInviteBySlug(slug);
  if (!invite) return null;
  return dispatch(slug, { type: "started" });
}

export function cancelInvite(slug: string, by: "host" | "system" = "host"): DirectInviteRecord | null {
  const invite = getInviteBySlug(slug);
  if (!invite) return null;
  if (invite.phase === "cancelled" || invite.phase === "expired" || invite.phase === "started") {
    return invite;
  }
  return dispatch(slug, { type: "cancel", by });
}
