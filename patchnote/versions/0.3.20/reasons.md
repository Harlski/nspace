# Reasons — 0.3.20 (patch-notes version)

**Patch-notes version:** `0.3.20` (frozen via `npm run prepare-merge`).
...
**[NEW]** **Prefab capture UX** — brighter footprint + block outlines; hover/drag previews; capture thumbnail in dock satellite + publish modal; `fitDesignPreviewRootInInspectorFrame` for dock/library thumbs; 6×6 bbox clamp (client + server).

**[CHANGE]** **Prefab dock chrome** — hide legacy Save/Place row and context panel when strip active; reset to **Terrain** tab on build close (`resetBuildDockCategoryToTerrain`).

**[NEW]** **Mobile prefab place** — `prefabPlaceArmedAnchor`, tap-to-preview/confirm, HUD Place (green) / Cancel (red); `rebuildPrefabPlaceMeshTemplate` on re-arm after place.

**[CHANGE]** **Design names** — max 12 characters (`designs.ts` + publish modal); library cards show creator identicon → profile.
