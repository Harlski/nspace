...
- [OPS] **Return Walk** needs `RESIDENT_ADDRESSES` (NimiqLIVE Resident wallet), `RETURN_WALK_SERVER_WALLET_ADDRESS` (Deposit destination; must not be the Stream Faucet `NQ21 F410 VXJB UK02 6TLG 8YHT 4M4B L664 NPM7`), and `NIM_RPC_URL` on the game server.
...
- [OPS] No `/api/admin/*` routes for this feature. Non-Resident JWTs get 403 on `/api/resident/*`.
