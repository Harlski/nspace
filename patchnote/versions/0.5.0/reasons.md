# Reasons — 0.5.0 (patch-notes version)

**Patch-notes version:** `0.5.0` (frozen via `npm run prepare-merge`).
...
**0.5.0** — Player Menu (tap navigation, long-press → Action Wheel), profile sheet with Wardrobe/Shop/Achievements tabs, paper-doll cosmetics + daily featured shop (gated by `SHOP_ENABLED`), The Shaper + return, full achievements system (v1 + v3 categories, banner, in-world celebration, Achievements window), Action Wheel glyph-only redesign, Hub/Commons display-name rename. Removed dev **Test achievement unlock** debug button.
...
- `client/src/ui/hud.ts` — mount new wardrobe; revert preview on profile close; **Player Menu** (`createPlayerMenu`), logout/leave handlers, Action Wheel mutual close, identicon sync from player bar; **long-press Player Menu trigger → Action Wheel**; removed `?achievementUnlockDebug` test unlock button.
- `client/src/ui/playerMenu.ts` — bottom-right identicon trigger + vertical popover list + inline logout/leave confirm; **long-press** on identicon/name pill opens Action Wheel (`onLongPress`, 480ms threshold).
...
- `server/test/cosmeticStore.test.ts`, `server/test/cosmeticGallery.test.ts`, `client/src/ui/playerMenu.test.ts` — daily featured selection, Shaper enablement, menu item shape, long-press gesture tests.

### Shop access gate

- `server/src/shopAccess.ts` / `client/src/cosmetics/shopAccess.ts` — `SHOP_ENABLED` / `VITE_SHOP_ENABLED`; COMING SOON when closed.
- `server/test/shopAccess.test.ts`, `client/src/cosmetics/shopAccess.test.ts` — gate tests.

### Overlay back stack + leave confirm

- `client/src/ui/overlayBackStack.ts` — browser Back closes nested overlays (profile, achievements window).
- `client/src/ui/leaveGameConfirm.ts` — confirm before leaving game session.

### Achievements v3 — exploration, social, football

- `server/src/explorationAchievementEvaluator.ts` — Marathon, Room Tourist, Grand Tour, teleporter/outfield milestones.
- `server/src/matchAchievementEvaluator.ts` — Clean Sheet, Comeback Kid, Golden Patience, etc.
- `server/src/fieldGoalAchievementEvaluator.ts` — Rush Hour, Daily Streak, Underdog Country.
- `client/src/achievements/celebrationPolicy.ts` / `achievementCelebrationVfx.ts` — in-world trophy pop timing, stagger, opacity.
- `client/src/achievements/panelData.ts` — Category Navigator labels/groups; Summary view data.
