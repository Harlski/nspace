# Reasons — 0.3.5 (patch-notes version)

**Patch-notes version:** `0.3.5` (frozen via `npm run prepare-merge`).
...
- **GitHub Actions → VPS** ([`.github/workflows/deploy-docker.yml`](../../../.github/workflows/deploy-docker.yml)): before `git fetch` / `reset` / `docker compose build` / `up`, the remote script runs **`docker compose stop`** (graceful **SIGTERM** to `nspace` so persistence flushes), then **`tar`** of host **`data/`** → **`backups/nspace-data-<UTC>.tar.gz`** under the deploy root (default `/opt/nspace`). Documented in [`docs/deploy-github-docker.md`](../../../docs/deploy-github-docker.md), [`docs/live-service-implementation.md`](../../../docs/live-service-implementation.md), [`docs/docker-deployment.md`](../../../docs/docker-deployment.md). Root [`.gitignore`](../../../.gitignore) ignores `backups/` for dev clones. Larger-system record: [`docs/THE-LARGER-SYSTEM.md`](../../../docs/THE-LARGER-SYSTEM.md) + [`docs/reasons/reason_458291.md`](../../../docs/reasons/reason_458291.md). Backlog note: in-game **“update incoming”** warning — [`docs/brainstorm/live-service-future.md`](../../../docs/brainstorm/live-service-future.md).
