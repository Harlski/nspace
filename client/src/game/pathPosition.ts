import { snapFloorTile } from "./grid.js";
...
/** Matches server {@link rooms.ts} ARRIVE_EPS for human path followers. */
export const PATH_ARRIVE_EPS = 0.04;

/** Matches server {@link rooms.ts} MOVE_SPEED. */
export const DEFAULT_PATH_MOVE_SPEED = 5;

/** Matches server {@link rooms.ts} TICK_MS. */
export const DEFAULT_PATH_TICK_MS = 50;

export type WaypointYFn = (layer: 0 | 1, gx: number, gz: number) => number;

function snapToTile(x: number, z: number): { x: number; z: number } {
  const t = snapFloorTile(x, z);
  return { x: t.x, z: t.y };
}
...
/**
 * One simulation step along a human path — same rules as server `advanceAlongPathHuman`.
...
/**
 * Pose at `nowMs` from `startAtMs`, stepping in `tickMs` slices (same cadence as the room tick loop).
 */
